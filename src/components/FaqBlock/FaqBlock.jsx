...
const SDFag = [
    ...
    },
    {
        ...
        content: <div>
            Как работает маскировка IP? Вы можете в любой момент изменить IP-адрес без передачи личных данных сторонним сервисам и лишней нагрузки на операционную систему смартфона и батарею.
        </div>
    },
    {
        title: "В каких странах находятся сервера для смены IP?",
        content: <div>
            Три страны на выбор: Польша, Германия, Турция.
        </div>
    },
    {
        title: "Что такое приоритетное подключение?",
        content: <div>
            При подключении смартфона к базовым станциям — как в какой-то точке в городе, так и на транспорте — всегда действуют настройки приоритизации. У пользователей Membrana в сети самый высокий приоритет — это значит, что они подключаются быстрее и скорость в сети у них более чем в 2 раза выше.
            Ресурсы базовой станции распределяются между клиентами в зоне её действия: подключаются все, просто с разной скоростью.
        </div>
    },
    {
        title: "Как работает секретарь в Membrana?",
        content: <div>
            Когда вы задали соответствующие настройки или когда просто не берёте трубку, на звонки в Мембране отвечает секретарь. Наш секретарь анализирует входящий вызов и принимает решение: отклонить сразу, поговорить самому и сохранить разговор или соединить с вами. Его задача — не только отсеять или принять на себя спам, но и передать вам всю существенную информацию. Для этого мы обучаем его на смоделированных и реальных диалогах.
    ...
    },
    {
        ...
        content: <div>
            Вы можете по очереди подключить до 3 дополнительных номеров. Используйте их для регистрации на различных интернет-ресурсах и в сервисах, чтобы не сообщать свой основной номер и не попадать под рекламный скоринг.
        </div>
    },
    {
        title: "Для чего нужны профили?",
        content: <div>
            По умолчанию создаются два профиля: «Общий» и «Не беспокоить», но вы можете создать любые. Например, «Дом», «Работа», «Встреча», «Отдых». В каждом профиле действуют свои настройки для групп контактов: кто может до вас дозвониться и для кого вы в этом режиме недоступны.
        </div>
    },
    {
        ...
        content: <div>
            ...
            В настройках профиля все номера сначала разделены на Неизвестные номера и Номера из списка контактов. Для доступа к списку контактов вам необходимо предоставить соответствующее разрешение в приложении. Разрешение можно предоставить еще на этапе регистрации, либо выдать позже при создании группы контактов или напрямую в настройках операционной системы смартфона.
            Вы можете создать свои группы контактов (например, «Семья», «Коллеги», «Друзья») и назначать им правила:
            — Напрямую принимать звонки: «На меня».
            — Отправлять звонки на Секретаря.
            — Сбрасывать: «Блок».
            Вы можете создать профили с разными правилами для одних и тех же групп контактов. Например, настроить профиль «Дом» таким образом, чтобы до вас могли дозвониться только друзья и родственники, а профиль «Работа» — чтобы дозвониться могли только коллеги и партнёры.
        </div>
    },
    {
        title: "Техподдержка",
        content: <div>
            По всем вопросам: futurecrew@mts.ru
        </div>
    },
]
const BlockContent = () => {
